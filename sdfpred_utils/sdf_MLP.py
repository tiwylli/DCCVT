...
class Decoder(torch.nn.Module):
    ...
    def pre_train_target_pc(self, iter, points):
        ...
        for i in tqdm(range(iter)):
            ...
            output = self(x_rand.detach())
            relative_l2_error = (output - target.to(output.dtype))**2 #/ (output.detach()**2 + 0.01)
            loss = relative_l2_error.mean()
            
            #loss on target points where self = 0
            # p = points
            # ref_value = torch.zeros((p.shape[0], 1), device=device, dtype=torch.float64)
            # output = self(p)
            # loss += loss_fn(output[...,0], ref_value)
