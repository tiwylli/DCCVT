...
def _alpha_shape_3d(points: np.ndarray, alpha: float):
    """
...
def _pick_alpha(points: np.ndarray, k: int = 8, quantile: float = 0.9, magnitude: float = 15.0) -> float:
...
def complex_alpha_sdf(mnfld_points: torch.Tensor, sites: torch.Tensor) -> torch.Tensor:
    points_np = mnfld_points.squeeze(0).detach().cpu().numpy()
    alpha = _pick_alpha(points_np)  # or set manually
    mesh = _alpha_shape_3d(points_np, alpha)
